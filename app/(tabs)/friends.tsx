      ...
      <ScrollView ref={scrollRef} style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            {searchResults.map((u) => (
              <View key={u.id} style={[styles.searchResultRow, { borderBottomColor: colors.border }]}>
                ...
                {sentRequests.has(u.id) ? (
                  <Text style={{ fontSize: 12, color: colors.textLight, fontWeight: '600' }}>Sent</Text>
                ) : (
                  <Pressable style={[styles.addBtnSmall, { backgroundColor: colors.accent }]} onPress={() => handleSendRequest(u.id)}>
          ...
          <>
            ...
            {requests.map((r) => (
              <View key={r.id} style={[styles.requestCard, { backgroundColor: colors.card }]}>
                <Avatar username={r.requester.username} color={r.requester.avatar_color} size={38} />
                ...
                <Pressable style={[styles.acceptBtn, { backgroundColor: colors.accent }]} onPress={() => handleAccept(r.id)}>
                  <Text style={{ color: '#FFF', fontSize: 12, fontWeight: '700' }}>Accept</Text>
                </Pressable>
                <Pressable style={styles.declineBtn} onPress={() => handleDecline(r.id)}>
        ...
        {friends.length === 0 ? (
          <View style={[styles.emptySection, { backgroundColor: colors.card }]}>
            <Text style={[styles.emptyText, { color: colors.textLight }]}>No friends yet. Search for a username or share your invite link to get started.</Text>
          </View>
        ) : (
          friends.map((f) => (
        ...
        {results.length > 0 && (
          <>
            <SectionLabel label="LAST 3 RESULTS" colors={colors} />
            {results.map((r) => {
