    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {activeCampaign === 'classic' ? (
          ...
          // Side campaign worlds — coming soon placeholder
          campaign.worldNames.map((name, i) => (
            <View key={i} style={[styles.worldCard, { backgroundColor: colors.card }]}>
              <View style={[styles.worldBadge, { backgroundColor: campaign.color + '14' }]}>
                <Text style={[styles.worldBadgeText, { color: campaign.color }]}>WORLD {i + 1}</Text>
              </View>
              <Text style={[styles.worldName, { color: colors.text }]}>{name}</Text>
              <Text style={[styles.worldSubtitle, { color: colors.textMid }]}>{campaign.levelsPerWorld[i]} levels</Text>
              <View style={[styles.comingSoonBadge, { backgroundColor: campaign.color + '10' }]}>
                <Text style={[styles.comingSoonText, { color: campaign.color }]}>Coming soon</Text>
              </View>
            </View>
          ))
        )}
