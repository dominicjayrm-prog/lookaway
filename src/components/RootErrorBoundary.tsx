...
export class RootErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    ...
    const isDev = Platform.OS !== 'web' && (typeof __DEV__ !== 'undefined' ? __DEV__ : false);
      ...
      <View style={styles.container}>
        ...
        {isDev && (
          <View style={styles.devBox}>
            <Text style={styles.devText}>{String(this.state.error?.message ?? this.state.error)}</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  devBox: { backgroundColor: '#FFF', padding: 12, borderRadius: 8, marginBottom: 16, maxWidth: 360 },
  devText: { fontSize: 11, fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace', color: '#FF6B6B' },
