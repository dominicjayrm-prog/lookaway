    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.backdrop}>
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          ...
          <Text style={[styles.title, { color: colors.text }]}>Out of lives!</Text>
          <Text style={[styles.countdown, { color: colors.textMid }]}>Next free life in {countdown}</Text>

          {/* Primary: £0.99 cash refill (most prominent) */}
          <Pressable style={[styles.cashButton, { backgroundColor: colors.accent }]} onPress={handleCashRefill}>
          ...
          <Pressable
            style={[styles.gemButton, { backgroundColor: colors.surface }, gems < LIVES_CONFIG.gemRefillCost && { opacity: 0.4 }]}
          ...
          >
            ...
            <Text style={[styles.gemButtonText, { color: colors.textMid }]}>Refill with gems</Text>
            <Text style={[styles.gemCost, { color: colors.textMid }]}>{String.fromCodePoint(0x1F48E)} {LIVES_CONFIG.gemRefillCost}</Text>
          </Pressable>

          {/* Dismiss */}
          <Pressable style={styles.ghostButton} onPress={onClose}>
            <Text style={[styles.ghostButtonText, { color: colors.textLight }]}>No thanks, I'll wait</Text>
          </Pressable>
        </View>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 20, padding: spacing.xxl, width: '100%', maxWidth: 340, alignItems: 'center' },
  heartsRow: { fontSize: 28, letterSpacing: 4, marginBottom: spacing.lg },
  title: { fontSize: 22, fontWeight: typography.weights.bold, marginBottom: spacing.sm },
  countdown: { fontSize: typography.sizes.md, fontWeight: typography.weights.medium, marginBottom: spacing.xxl },
  ...
  gemButtonText: { flex: 1, fontSize: typography.sizes.md, fontWeight: typography.weights.medium },
  gemCost: { fontSize: typography.sizes.sm, fontWeight: typography.weights.semibold },

  ghostButton: { paddingVertical: spacing.sm },
  ghostButtonText: { fontSize: typography.sizes.md, fontWeight: typography.weights.medium },
});
