...
function loadState(): any {
  try { if (typeof window === 'undefined') return {}; const saved = localStorage.getItem('lookaway-progress'); return saved ? JSON.parse(saved) : {}; } catch { return {}; }
}
function saveState(state: any) {
  try { if (typeof window === 'undefined') return; localStorage.setItem('lookaway-progress', JSON.stringify({ gems: state.gems, lives: state.lives, maxLives: state.maxLives, livesLastLostAt: state.livesLastLostAt, streakCount: state.streakCount, totalStars: state.totalStars, highestWorld: state.highestWorld, levelProgress: state.levelProgress, completedScores: state.completedScores })); } catch {}
}
...
export const useGameStore = create<GameStore>((set, get) => {
  ...
  return {
    gems: saved.gems ?? 100, lives: saved.lives ?? 5, maxLives: saved.maxLives ?? 5, livesLastLostAt: saved.livesLastLostAt ?? null, streakCount: saved.streakCount ?? 0, totalStars: saved.totalStars ?? 0, highestWorld: saved.highestWorld ?? 1,
    levelProgress: saved.levelProgress ?? {}, completedScores: saved.completedScores ?? [],
    currentLevel: null, gameState: 'READY' as GameState, currentSceneIndex: 0, currentQuestionIndex: 0, answers: [] as Answer[], selectedOption: null, revealedCorrect: null, score: 0, _hydrated: false,
